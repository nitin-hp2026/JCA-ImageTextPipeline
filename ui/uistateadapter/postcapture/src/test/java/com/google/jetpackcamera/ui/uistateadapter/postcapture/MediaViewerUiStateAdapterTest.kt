...
class MediaViewerUiStateAdapterTest {
    ...
    @Test
    fun mediaViewerUiState_fromMediaImageWithRecognizedText_returnsImageWithTextState() {
        val mediaDescriptor = MediaDescriptor.Content.Image(testUri, testBitmap)
        val media = Media.Image(testBitmap)
        val recognizedTextState = ImageTextRecognitionState.Ready(
            RecognizedTextResult(
                ...
                words = listOf(
                    RecognizedTextWord(
                        text = "Hello",
                        boundingBox = Rect(0, 0, 20, 20),
                        lineIndex = 0,
                        wordIndexInLine = 0
                    ),
                    RecognizedTextWord(
                        text = "world",
                        boundingBox = Rect(22, 0, 50, 20),
                        lineIndex = 0,
                        wordIndexInLine = 1
                    )
                )
        ...
        val actualUiState = MediaViewerUiState.from(
            mediaDescriptor = mediaDescriptor,
            media = media,
            player = null,
            playerState = false,
            imageTextRecognitionState = recognizedTextState
        )

        assertThat(actualUiState).isEqualTo(
            MediaViewerUiState.Content.Image(
