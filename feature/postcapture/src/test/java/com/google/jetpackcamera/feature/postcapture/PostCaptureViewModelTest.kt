...
import com.google.jetpackcamera.data.media.RecognizedTextResult
import com.google.jetpackcamera.data.media.RecognizedTextWord
import com.google.jetpackcamera.data.media.testing.FakeImageTextRecognitionRepository
...
internal class PostCaptureViewModelTest {
    ...
    private lateinit var imageTextRecognitionRepository: FakeImageTextRecognitionRepository
    ...
    fun setup() = runTest(testDispatcher) {
        ...
        imageTextRecognitionRepository = FakeImageTextRecognitionRepository()
        ...
        viewModel = PostCaptureViewModel(
            ...
            imageTextRecognitionRepository = imageTextRecognitionRepository,
            context = testContext
        )
        advanceUntilIdle()
    }
    ...
    fun getUiState_image_viewerHasContent() = runTest(testDispatcher) {
        ...
        assertThat(imageTextRecognitionRepository.startRecognitionCalls).contains(testImageUri)
    }

    @Test
    fun getUiState_image_usesCachedRecognizedText() = runTest(testDispatcher) {
        imageTextRecognitionRepository.emitState(
            testImageUri,
        ...
        val uiState = viewModel.postCaptureUiState.first {
            it is PostCaptureUiState.Ready &&
                (it.viewerUiState as? MediaViewerUiState.Content.Image)?.imageTextUiState is
                ImageTextUiState.Ready
        } as PostCaptureUiState.Ready

        val textUiState =
            (uiState.viewerUiState as MediaViewerUiState.Content.Image).imageTextUiState
                as ImageTextUiState.Ready
        assertThat(textUiState.fullText).isEqualTo("Detected text")
    }

    @Test
    fun getUiState_image_noDetectedText_mapsToEmpty() = runTest(testDispatcher) {
        imageTextRecognitionRepository.emitState(testImageUri, ImageTextRecognitionState.Empty)
        mediaRepository.setCurrentMedia(testImageDesc)

        val uiState = viewModel.postCaptureUiState.first {
            it is PostCaptureUiState.Ready &&
                (it.viewerUiState as? MediaViewerUiState.Content.Image)?.imageTextUiState ==
                ImageTextUiState.Empty
        } as PostCaptureUiState.Ready

        val viewerState = uiState.viewerUiState as MediaViewerUiState.Content.Image
        assertThat(viewerState.imageTextUiState).isEqualTo(ImageTextUiState.Empty)
    }
    ...
    @Test
    fun saveCurrentMedia_cachedImage_onSuccess_copiesRecognitionToSavedUri() =
        runTest(testDispatcher) {
            val savedUri = Uri.parse("file:///saved-image.jpg")
            mediaRepository.setCurrentMedia(testCacheImageDesc)
            mediaRepository.saveToMediaStoreHandler = { savedUri }
            advanceUntilIdle()

            viewModel.saveCurrentMedia()
            advanceUntilIdle()

            assertThat(imageTextRecognitionRepository.copiedRecognitions)
                .contains(testImageUri to savedUri)
        }
