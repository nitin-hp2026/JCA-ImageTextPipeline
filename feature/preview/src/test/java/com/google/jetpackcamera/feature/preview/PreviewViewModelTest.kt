...
class PreviewViewModelTest {
    ...
    private val imageTextRecognitionRepository = FakeImageTextRecognitionRepository()
    ...
    @Test
    fun captureImage_withSavedUri_startsImageTextRecognition() = runTest(StandardTestDispatcher()) {
        val savedUri = Uri.parse("file:///captured-image.jpg")
        val captureCameraSystem = object : CameraSystem by cameraSystem {
            ...
            override suspend fun takePicture(
                ...
                cameraSystem.takePicture(contentResolver, saveLocation, onCaptureStarted)
                return ImageCapture.OutputFileResults(savedUri)
            }
        }
        val localPreviewViewModel = PreviewViewModel(
            cameraSystemRepository = object : CameraSystemRepository {
                override val cameraSystem = captureCameraSystem
            },
            constraintsRepository = constraintsRepository,
            settingsRepository = FakeSettingsRepository,
            mediaRepository = FakeMediaRepository(),
        ...
        startCameraUntilRunning(localPreviewViewModel)
        localPreviewViewModel.captureController.captureImage(contentResolver)
        advanceUntilIdle()

        assertThat(imageTextRecognitionRepository.startRecognitionCalls).contains(savedUri)
    }
